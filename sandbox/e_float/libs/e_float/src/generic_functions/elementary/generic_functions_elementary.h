...
// in ACM TOMS, {VOL 37, ISSUE 4, (February 2011)} (C) ACM, 2011. http://doi.acm.org/10.1145/1916461.1916469
  ...
  {
    template<typename T> const T& pi (void); // Forward declaration.
    template<typename T> const T& ln2(void); // Forward declaration.

    template<typename T> double frexp10  (const T& x, INT64* ptrexp10);
    template<typename T> T      pown     (const T& t, const INT64 p);
    template<typename T> T      sqrt     (const T& x);
    template<typename T> T      rootn    (const T& x, const INT32 p);
    ...
    template<typename T> bool isneg    (const T& x) { return (x < static_cast<INT32>(0)); }
    ...
    template<> float       rootn<float>      (const float& x,       const INT32 p) { return ::powf(x, static_cast<float>(p)); }
    template<> double      rootn<double>     (const double& x,      const INT32 p) { return ::pow(x, static_cast<double>(p)); }
    template<> long double rootn<long double>(const long double& x, const INT32 p) { return ::pow(x, static_cast<long double>(p)); }
    template<> e_float     rootn<e_float>    (const e_float& x,     const INT32 p) { return ef::rootn(x, p); }
    ...
    template<typename T> T sqrt(const T& x)
    {
      ...
      if(static_cast<INT64>(ne % static_cast<INT32>(2)) != static_cast<INT64>(0u))
      {
        ne += 1;
        dd /= 10.0;
      }
      ...
      for(INT32 digits = std::numeric_limits<double>::digits10 - 1; digits <= std::numeric_limits<T>::digits10; digits *= 2)
      {
        // Next iteration of vi
        vi += vi * (-((result * vi) * static_cast<INT32>(2)) + static_cast<INT32>(1));
    ...
    template<typename T> T rootn(const T& x, const INT32 p)
    {
      ...
      {
        return generic_functions::rootn<T>(static_cast<INT32>(1) / x, static_cast<INT32>(-p));
      }

      if((p == static_cast<INT32>(0)) || generic_functions::isneg(x))
      {
        return std::numeric_limits<e_float>::quiet_NaN();
      }
      ...
      {
        // Next iteration
        T term = (((-generic_functions::pown<T>(rtn, p) * x) + static_cast<INT32>(1)) / p) + static_cast<INT32>(1);
        rtn *= term;
      }

      return static_cast<INT32>(1) / rtn;
    }

    template<typename T> bool small_arg(const T& x)
    {
      // Values less than the sixth root of epsilon are considered small.
      static const T val_small = generic_functions::rootn<T>(std::numeric_limits<T>::epsilon(), static_cast<INT32>(6));
    ...
    template<typename T> T log(const T& x)
    {
      ...
      const T xx = ((!b_neg) ? x : static_cast<INT32>(1) / x);
      ...
      T ak(1u);
      T bk = static_cast<INT32>(4) / (xx * generic_functions::pown<T>(T(2), static_cast<INT64>(m)));
      ...
      const T val =   (generic_functions::pi<T>() / (ak * static_cast<INT32>(2)))
...
#endif // _GENERIC_FUNCTIONS_ELEMENTARY_2011_06_18_H_
